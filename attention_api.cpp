...
torch::Tensor attention(torch::Tensor q, torch::Tensor k, torch::Tensor v, torch::Tensor k_cache, torch::Tensor v_cache, float softmax_scale, int curr_timestep, torch::Tensor seq_lengths)
{
    ...
    if (q.scalar_type() == torch::ScalarType::Float){
        using T = float;
        call_attention<T>(batch_size, head_num, head_dim, seq_length, softmax_scale, curr_timestep, q.data_ptr<T>(), k.data_ptr<T>(), v.data_ptr<T>(), k_cache.data_ptr<T>(), v_cache.data_ptr<T>(), out.data_ptr<T>(), seq_lengths.data_ptr<int>());
    }
    else if (q.scalar_type() == torch::ScalarType::Half){
        using T = uint16_t;
        //T* a = static_cast<T*>(q.data_ptr());
        //auto a = q.data_ptr();
        call_attention<T>(batch_size, head_num, head_dim, seq_length, softmax_scale, curr_timestep, reinterpret_cast<T*>(q.data_ptr<at::Half>()), reinterpret_cast<T*>(k.data_ptr<at::Half>()), reinterpret_cast<T*>(v.data_ptr<at::Half>()), reinterpret_cast<T*>(k_cache.data_ptr<at::Half>()), reinterpret_cast<T*>(v_cache.data_ptr<at::Half>()), reinterpret_cast<T*>(out.data_ptr<at::Half>()), seq_lengths.data_ptr<int>());
    }
    else
        printf("unsupported tensort type\n");
...
torch::Tensor attention(torch::Tensor q, torch::Tensor k, torch::Tensor v, torch::Tensor k_cache, torch::Tensor v_cache, float softmax_scale, int curr_timestep)
{
    ...
    if (q.scalar_type() == torch::ScalarType::Float){
        using T = float;
        call_attention<T>(batch_size, head_num, head_dim, seq_length, softmax_scale, curr_timestep, q.data_ptr<T>(), k.data_ptr<T>(), v.data_ptr<T>(), k_cache.data_ptr<T>(), v_cache.data_ptr<T>(), out.data_ptr<T>(), nullptr);
    }
    else if (q.scalar_type() == torch::ScalarType::Half){
        using T = uint16_t;
        //T* a = static_cast<T*>(q.data_ptr());
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
      m.def("attention", py::overload_cast<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, float, int>(&attention), "Attention forward (CUDA) fixed timesteps");
      m.def("attention", py::overload_cast<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, float, int, torch::Tensor>(&attention), "Attention forward (CUDA) varied timesteps");
