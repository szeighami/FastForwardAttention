    ...
    def forward(
        ...
        if use_ff_attention:
            ...
            if query.shape[-2] == 1:
                ...
                ffa_cache = layer_past[2]
                timestep = layer_past[3]
                prompt_seqlens = layer_past[4]
                k_cache, v_cache = ffa_cache.get_cache()
                attn_output = ffa_cache.attention(query.to(value.dtype), key.to(value.dtype), value, k_cache, v_cache, 1/self.scale_attn, timestep, prompt_seqlens, 0)

                present = (k_cache, v_cache, ffa_cache, timestep+1, prompt_seqlens)

                attn_output = attn_output.reshape(query.shape)
            else:
                max_prompt_seqlen = key.shape[-2]
                max_seqlen = max_prompt_seqlen+max_tokens

                ffa_cache = ffa(key.to(value.dtype).contiguous(), value.contiguous(), max_seqlen, use_ff_attention_default)
                k_cache, v_cache = ffa_cache.get_cache()

                seq_lens = torch.max(attention_mask[:, :, -1, :], dim=-1).indices
                
                present = (k_cache, v_cache, ffa_cache, max_prompt_seqlen, seq_lens.reshape(-1).int())
